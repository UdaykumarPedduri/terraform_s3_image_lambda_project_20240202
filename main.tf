...
resource "aws_instance" "my_instance-one" {
  ami           = "ami-0ca9fb66e076a6e32" # Use the appropriate AMI ID
  instance_type = "t2.micro"
  key_name      = "devops-practice"
  tags = {
    Name = "Ec2InstancebyUdayP-GD"
  }
}
# Create an S3 bucket
resource "aws_s3_bucket" "s3_bucket" {
  bucket = "source-image-bucket-ukpedduri"  # Ensure this name is globally unique
  ...
  logging {
    target_bucket = "source-image-bucket-ukpedduri"
    target_prefix = "log/"
  }
  ...
  # Tags for the bucket
  tags = {
    Name        = "source-image-bucket-ukpedduri"
...
data "archive_file" "lambda_zip" {
  type        = "zip"
  source_file = "/workspaces/terraform_s3_image_lambda_project_20240202/lambda_function.py"
  output_path = "/workspaces/terraform_s3_image_lambda_project_20240202/lambda_function.zip"
}
